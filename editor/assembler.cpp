...
namespace NEONnoir
{
    assembler::assembler(std::string const& source, std::shared_ptr<game_data> data) : _source(source), _game_data(data)
    {
    ...
    assembler_result assembler::assemble()
    {
        ...
        return { _bytecode, _scripts, _flags, _constants };
    }
            ...
            if (is_alpha(peek()))
            {
                ...
                for (auto type : _opcodes[name].params)
                {
                    ...
                    case ParamType::TEXT:
                    {
                        ...
                        if (!_game_data->strings.constains_string(string_id))
                        {
